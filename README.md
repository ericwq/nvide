...
```sh
docker pull ericwq057/nvide:0.9.3
docker run -it -d -h nvide --env TZ=Asia/Shanghai --name nvide ericwq057/nvide:0.9.3
...
With the base image, ssh keys and passwords in hands, it's time to build the `sshd-lazy` image. Please note `sshd-lazy` image is based on `ericwq057/nvide:0.9.3`. Base image is required to build the `ssh-lazy` image.
...
docker build --build-arg ROOT_PWD=password \
	...
	--progress plain -t sshd-lazy:0.11.3 -f sshd-lazy.dockerfile .
```
...
docker run --env TZ=Asia/Shanghai --tty --privileged \
    ...
    -h sshd-lazy --name sshd-lazy -d -p 22:22  sshd-lazy:0.11.3
```
