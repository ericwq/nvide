...
# 安装 Go 1.26
RUN curl -LO https://go.dev/dl/go1.26.2.linux-${ARCH}.tar.gz && \
  rm -rf /usr/local/go && \
  tar -C /usr/local -xzf go1.26.2.linux-${ARCH}.tar.gz && \
  rm go1.26.2.linux-${ARCH}.tar.gz
...
RUN curl -L https://github.com/tree-sitter/tree-sitter/releases/latest/download/tree-sitter-linux-${ARCH}.gz | \
